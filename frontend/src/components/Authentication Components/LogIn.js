...
const Login = () => {
  ...
  const [rememberMe, setRememberMe] = useState(true); // Default to true for permanent login
  const navigate = useNavigate();

  const BACKEND_URL =
    process.env.REACT_APP_BACKEND_URL ||
    process.env.REACT_APP_DEPLOYED_BACKEND_URL;
      ...
      <div className="relative inset-0 flex items-center justify-center pt-20 md:pt-44 lg:pt-10">
        {!otpSent ? (
          <form
            ...
            className="w-full md:px-32 lg:px-[330px] p-8"
          >
            ...
            <button
              ...
              className={`w-full font-bold bg-red-600 text-white py-2 px-4 text-2xl rounded-md hover:bg-red-700 transition-colors ${
          ...
          <div className="w-full max-w-md p-8">
            ...
            <form onSubmit={handleOtpVerify} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={otpLoading}
                className={`w-full bg-red-600 text-white text-xl md:text-2xl py-2 px-4 rounded-md hover:bg-red-700 font-bold transition-colors ${
